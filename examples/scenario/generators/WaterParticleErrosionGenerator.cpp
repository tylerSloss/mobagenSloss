...
#include "WaterParticleErrosionGenerator.h"
#include "../FastNoiseLite.h"
#include "Random.h"
#include <iostream>
#include <iomanip>

std::vector<Color32> WaterParticleErrosionGenerator::Generate(int sideSize, float time) {
  // bootstrap condition
...
std::string WaterParticleErrosionGenerator::GetName() { return "WaterParticle-Based Hydraulic Erosion"; }
std::vector<Color32> WaterParticleErrosionGenerator::heightsToColor() {
...
void WaterParticleErrosionGenerator::generateRandomHeights() {
...
glm::vec3 WaterParticleErrosionGenerator::surfaceNormal(int i, int j) {
  float scale = 10;
...
glm::vec2 WaterParticleErrosionGenerator::surfaceNormalVec2(int i, int j){
  float scale = 15;
  float diagnolScale = 10;

  auto point = heights[i][j];
  auto north = heights[i][j+1];
  auto south = heights[i][j-1];
  auto left = heights[i-1][j];
  auto right = heights[i+1][j];
  auto nl = heights[i-1][j+1];
  auto nr = heights[i+1][j+1];
  auto sl = heights[i-1][j-1];
  auto sr = heights[i+1][j-1];

  glm::vec2 vwf = glm::vec2( right - left, north - south) * scale;
  vwf += glm::vec2(nr - sl,nl - sr) * diagnolScale;
...
void WaterParticleErrosionGenerator::Erode(float dt) {
  ...
  for(int i = 0; i < cycles; i++){
    ...
    while(drop.volume > minVol){
      ...
      glm::vec2 m = surfaceNormalVec2(ipos.x, ipos.y);

      //Accelerate WaterParticle using newtonian mechanics using the surface normal.
      drop.speed += dt*glm::vec2(m.x, m.y)/(drop.volume*density);//F = ma, so a = F/m
